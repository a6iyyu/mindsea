<section class="mb-12">
  <div class="bg-[#fceede]/30 rounded-xl p-8 border-4 border-[#f58a66]/20">
    <h2 class="text-2xl font-bold text-center text-gray-800 mb-6">Tim Kami</h2>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      @foreach($team as $member)
        <article class="bg-white p-6 rounded-xl text-center">
          <img src="{{ $member['foto'] }}" alt="{{ $member['nama'] }}" class="w-24 h-24 mx-auto mb-4 rounded-full">
          <h3 class="text-lg font-semibold text-gray-800">{{ $member['nama'] }}</h3>
          <p class="mt-3 text-gray-600">{{ $member['jabatan'] }}</p>
        </article>
      @endforeach
    </div>
  </div>
