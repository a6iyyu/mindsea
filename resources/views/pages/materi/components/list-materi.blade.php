<section class="grid grid-cols-1 md:grid-cols-2 gap-8">
    @foreach($materi as $item)
        <div class="rounded-xl border-4 {{ $item->progress && $item->progress->is_completed ? 'border-gray-200 bg-gray-50' : 'border-blue-200 bg-white' }} p-8 shadow-lg transition-all hover:shadow-xl focus-within:ring-4 focus-within:ring-blue-200">
            ...
            @if($item->progress && $item->progress->is_completed)
                <div class="flex items-center gap-2 text-gray-500">
                    ...
                    <span class="text-sm">({{ $item->progress->completed_at->diffForHumans() }})</span>
                </div>
            @else
