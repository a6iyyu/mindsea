<section class="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
    @foreach($materi as $item)
        <article
            class="rounded-xl border-4 {{ $item->progress && $item->progress->is_completed ? 'border-gray-200 bg-gray-50' : 'border-blue-200 bg-white' }} p-8 shadow-lg transition-all hover:shadow-xl focus-within:ring-4 focus-within:ring-blue-200">
            ...
            <header class="mb-6 flex flex-col lg:flex-row items-center justify-between">
                <div class="flex items-center gap-4">
                    <span
                        class="{{ $item->progress && $item->progress->is_completed ? 'bg-gray-100' : 'bg-blue-100' }} rounded-xl p-5">
                        <i
                            class="fa-solid fa-book {{ $item->progress && $item->progress->is_completed ? 'text-gray-500' : 'text-blue-500' }} text-3xl"></i>
                    </span>
                </div>
            ...
            <h3
                class="mb-4 text-2xl font-bold {{ $item->progress && $item->progress->is_completed ? 'text-gray-600' : 'text-gray-800' }}">
                {{ $item->title }}
            </h3>
            <p
                class="mb-6 text-lg leading-relaxed {{ $item->progress && $item->progress->is_completed ? 'text-gray-500' : 'text-gray-600' }}">
                {{ $item->description }}
            </p>
